...
for alpha in alpha_list:    
    for layers in range(1,maxlayers+1):        
        for i, jz in enumerate(jz_values):
            ...
            vstate = nk.vqs.MCState(sampler, model=RBM, n_samples=alpha)

            # Definición de paths incluyendo layers y heads
            path_metrics = f'RBM_metrics{layers}_{jz:.2f}_{alpha}.csv'
            #filename = f"RBM{layers}_{jz:.2f}_{alpha}.mpack"
            #vstate_path = f"vstate_RBM{layers}_{jz:.2f}_{alpha}.pkl"
            obs_path = f'RBM_obs_layers{layers}_{alpha}.csv'
