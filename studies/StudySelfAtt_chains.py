...
n_chains_list = [16,32,64]
for chains in n_chains_list:
        ####REGLAS d MonteCarlo
    rule1 = nk.sampler.rules.LocalRule()# flip de un solo spin para proponer nuevo estado
    rule2 = nk.sampler.rules.GaussianRule()# flip de todos los spins para proponer nuevo estado
    rules = [rule1]
    sampler = nk.sampler.MetropolisSampler(
        hi, nk.sampler.rules.MultipleRules([rule1],[1.0]), n_chains = chains
    )    
    ...
    for layers in range(1,maxlayers+1):
        for heads in range(1,maxheads+1):
            for i, jz in enumerate(jz_values):
                ...
                path_metrics = f'SelfAtt_metrics{layers}_head{heads}_{jz:.2f}_{chains}.csv'
                #filename = f"SelfAtt{layers}_head{heads}_{jz:.2f}_{chains}.mpack"
                #vstate_path = f"vstate_SelfAtt{layers}_head{heads}_{jz:.2f}_{chains}.pkl"
                obs_path = f'obs_layers{layers}_head{heads}_{chains}.csv'
